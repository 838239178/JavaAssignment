function jumpToTop(sender) {
    console.log("click top");
    moveScroll(0, 300);
}

function createPassage() {
    var str = "“我出生的地方叫作三明，这里很多的网红，不止沙县和大田。城里的钢铁厂，美得像花园。麒麟山的脚下，就着擂茶吃拌面……”\n" +
    ...
    for (i = 0; i < list.length; i++) {
        ...
        var element = document.getElementsByClassName("passage-container")[0];
        //console.log(element);
...
function drawSnowCanvas() {
    ...
    image.src="../../img/flake.png";
    if(typeof(snowTimer) == "undefined") {
        snowTimer = setInterval(function () {
            g.clearRect(0,0, canvas.width, canvas.height);
            var count = getRandomNum(10,20);
            for (i=0; i<count; i++) {
                p.x = getRandomNum(0, canvas.width);
                p.y = getRandomNum(0, canvas.height);
                size.width = getRandomNum(10,100);
                size.height = size.width;
                g.drawImage(image, p.x, p.y, size.width, size.height);
            }
        },1000);
    } else {
...
function fixedDom(dom, height, fixedTop) {
    var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
    if(scrollTop > height){
        dom.style.position = "fixed";
        dom.style.top = fixedTop + "px";
    } else {
...
function setLocalTIme() {
    ...
    var data = new Date();
    obj.innerText = data.toLocaleTimeString();
